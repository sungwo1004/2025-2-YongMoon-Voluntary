...
class PongEnv:
    def __init__(self, render_mode=None):
        ...
        if render_mode == 'human':
            try:
                ...
                pygame.display.set_caption("Pong 게임") # 게임 이름 설정
    ...
    def step(self, action):
        ...
        # action이 0이면 왼쪽으로 0.03 이동
        # action이 2이면 오른쪽으로 0.03 이동
        # hint: self.paddle_x를 증가 또는 감소시키세요

        # 패들 이동 (좌우)
        if action == 0:
            self.paddle_x -= 0.03  # 왼쪽
        elif action == 2:
            self.paddle_x += 0.03  # 오른쪽
    ...
    def render(self):
        ...
        # Miss 횟수 표시
        miss_text = self.small_font.render(f"Miss: {self.miss_count}", True, (200, 200, 200))
...
if __name__ == "__main__":
    ...
    while running:
        ...
        if steps % 100 == 0:
            ...
            print(f"Score: {info['score']}, Miss: {info['miss_count']}")
        ...
        if done:
            ...
            print(f"최종 점수 - Score: {info['score']}, Miss: {info['miss_count']}\n")
            state = env.reset()
            total_reward = 0
            steps = 0
